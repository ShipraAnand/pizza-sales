...
FROM
    ...
    GROUP BY orders.order_data) AS data;
...
SELECT 
    ...
    order_details
        JOIN
    pizzas ON order_details.pizza_id = pizzas.pizza_id)*100 as totalrevenue
FROM
    pizza_types
        JOIN
    pizzas ON pizza_types.pizza_type_id = pizzas.pizza_type_id
...
select order_data, 
	sum(revenue) over (order by order_data)as cum_revenue
from
